...
export const updateProfile =(data)=>async(dispatch)=>{
    ...
    try {
        const response = await axios.put("http://localhost:2000/api/v1/user/update/profile",data,{
...
export const updatePassword =(data)=>async(dispatch)=>{
    ...
    try {
        const response = await axios.put("http://localhost:2000/api/v1/user/update/password",data,{
            withCredentials: true,
            headers:{"Content-type" : "application/json"},
        });
        dispatch(updateProfileSlice.actions.updatePasswordSuccess());
    } catch (error) {
        dispatch(updateProfileSlice.actions.updatePasswordFailed(error.response.data.message || "failed to update Profile"))
    }
};
